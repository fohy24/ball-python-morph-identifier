...
USE_COMPILE = True  # set to False on Windows-without-Triton for local smoke tests
...
if __name__ == '__main__':
    ...
    # --- Data loading ---
    labels_extended = pd.read_csv('data/top37_gene_df.csv')
    if SUBSET:
    ...
    for epoch_offset in range(1, FINETUNE_EPOCHS + 1):
        ...
        pd.DataFrame(result).to_csv(
            f'results/version{SAVE_AS_VERSION}_ImageSize{IMAGE_SIZE}_LR{LEARNING_RATE}.csv',
            index=False,
        )
    ...
    bn_dataset = PythonGeneDataset(
        labels_df=train_df, img_dir='data/img/',
